...
**Model name rule:** 对用户使用 API 的 display_name：**Seedream V5.0 Lite** / **Wan 2.7 Image Pro** / **Wan 2.7 Image**。
...
| Priority | model_id | Display Name | Mode | Resolution | Max Ref | Ratios | Notes |
|----------|----------|-------------|------|------------|---------|--------|-------|
| 1 | `seedream` | Seedream V5.0 Lite | sync | 2K, 3K | 14 | 21:9 16:9 3:2 4:3 1:1 3:4 2:3 9:16 | 同步出图，最快 |
| 2 | `wan2.7-image` | Wan 2.7 Image | async | 1K, 2K | 9 | 21:9 16:9 3:2 4:3 1:1 3:4 2:3 9:16 | 异步，需轮询 |
| 3 | `wan2.7-image-pro` | Wan 2.7 Image Pro | async | 1K, 2K, 4K | 9 | 21:9 16:9 3:2 4:3 1:1 3:4 2:3 9:16 | 异步，支持 4K |

Default `model`: **`seedream`**. Fallback: `wan2.7-image` → `wan2.7-image-pro`. Call `models.py` **only** when retrying after `failed` or user names a model you must validate.

### Quality priority（用户要求"最好/最高质量"时）

| Quality | model_id | Display Name |
|---------|----------|-------------|
| 1 | `wan2.7-image-pro` | Wan 2.7 Image Pro |
| 2 | `wan2.7-image` | Wan 2.7 Image |
...
**Defaults** (omit if unchanged): `model`=`seedream`, `aspect_ratio`=`1:1`, `resolution`=`2K`, `image_count`=`1`, `save_to_assets`=`true`.

| Intent | Add field |
|--------|-----------|
| Portrait | `"aspect_ratio":"9:16"` (or `2:3` / `3:4`) |
| Landscape | `"aspect_ratio":"16:9"` (or `3:2` / `4:3` / `21:9`) |
| Resolution | `"resolution":"3K"` / `"1K"` / `"4K"` (model-dependent, see table) |
...
`{"prompt":"电商主图，护肤品","model":"wan2.7-image-pro","aspect_ratio":"1:1","resolution":"4K"}`
...
`python scripts/image/submit.py '<json>'`
...
`python scripts/image/download.py '<url_or_array>' '<short_name>'`
...
- **Model name in output**: Use display_name only (Seedream V5.0 Lite / Wan 2.7 Image Pro / Wan 2.7 Image).
